...
class MarkOut extends Component {
  static propTypes = {
    ...
    lineWrapper: PropTypes.oneOfType([PropTypes.func, PropTypes.string])
  };
  getWrapper = ({ children }) => {
  ...
  render() {
    ...
    return <Wrapper>{lines.map(this.renderLine)}</Wrapper>;
  }
}
...
export const LineBreaker = ({ source, wrapper, lineWrapper }) => {
  ...
  return <Wrapper>{lines.map(renderLine)}</Wrapper>;
};
