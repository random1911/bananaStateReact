...
class Dropdown extends Component {
  ...
  render() {
    const {useTranslate, customHead, items, onSelect, selected, headerWidth, type, headerText} = this.props
    ...
    return (
      <Fragment>
        {CustomHeader ? (
          <CustomHeader innerRef={this.headElement} onClick={this.open} open={isOpen} width={headerWidth}>
            {useTranslate ? <Translate id={displayValue} /> : displayValue}
          </CustomHeader>
        ) : (
          <Head innerRef={this.headElement} onClick={this.open} width={headerWidth}>
            <CurrentValue>{useTranslate ? <Translate id={displayValue} /> : displayValue}</CurrentValue>
            <Arrow open={isOpen} />
          </Head>
        )}
        {isOpen && (
          <DropdownDrop closeCallback={this.close} params={dropdownPosition}>
            {
              type === 'select' && <SelectDrop closeCallback={this.close} items={items} onSelect={onSelect} selectedId={selected.id} />
            }
            {
